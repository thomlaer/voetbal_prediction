...
if [[ "${UPDATE_SOCCERBASE:-1}" == "1" ]]; then
  if [[ "${GITHUB_ACTIONS:-}" == "true" ]]; then
    ...
    SOCCERBASE_LOOKBACK_DAYS="${SOCCERBASE_LOOKBACK_DAYS:-0}"
    SOCCERBASE_MAX_TOURNAMENTS="${SOCCERBASE_MAX_TOURNAMENTS:-1}"
    SOCCERBASE_MAX_FETCH_GAMES="${SOCCERBASE_MAX_FETCH_GAMES:-0}"
    SOCCERBASE_WORLDCUP_DATE_PAGES="${SOCCERBASE_WORLDCUP_DATE_PAGES:-2026-06-11:2026-07-19}"
  fi
  SOCCERBASE_ARGS=(
    --skip-errors \
    --incremental \
